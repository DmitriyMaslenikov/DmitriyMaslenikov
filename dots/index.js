...
function expectLength(arr, longString, str) {
    ...
    if (uniqueValues.size === longString) {
        ...
        throw new Error("Строка -" + str + "  Размер массива - " + String(arr.length) + " несоответствует ожидаемому - " + String(longString));
    }
}
...
arrString.forEach(string => expectLength(arrLine(string), 2 ** (string.length - 1), string));
...
const arrString2 = [
    ...
    ['abcd', ['abcd', 'a.bcd', 'ab.cd', 'a.b.cd', 'abc.d', 'ab.c.d', 'a.bc.d', 'a.b.c.d']]
];

arrString2.forEach(arr => expectArraysAreEqual(arrLine(arr[0]), arr[1], arr[0]));
